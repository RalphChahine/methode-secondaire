...
const PAYMENT_METHOD_OPTIONS = ["stripe_payment_link", "demo", "interac", "cash", "other"];
...
function syncConfirmedSessionsToCalendar() {
  ...
  for (let rowIndex = 1; rowIndex < values.length; rowIndex += 1) {
    ...
    try {
      const calendar = resolveCalendarForTutor_(spreadsheet, row[columns.tutor_id]);
      if (!calendar) {
        errors.push(`Row ${rowIndex + 1}: tutor calendar is not available to the team account`);
        continue;
      }
      ...
      sheet.getRange(rowIndex + 1, columns.calendar_invites_sent_at + 1).setValue(new Date().toISOString());
...
function createPaymentRowsForScheduledSessions() {
  ...
  for (let rowIndex = 1; rowIndex < sessionValues.length; rowIndex += 1) {
    ...
    if (paymentDetails.payment_link && !normalizeValue_(row[sessionColumns.payment_link])) {
      sessionSheet.getRange(rowIndex + 1, sessionColumns.payment_link + 1).setValue(paymentDetails.payment_link);
    }
    ...
    const isDemoPayment = !paymentDetails.payment_link &&
      normalizeValue_(row[sessionColumns.payment_status]) === "payment_requested" &&
      Number(paymentDetails.amount_cad) > 0;
    const payment = {
      ...
      payment_method: paymentDetails.payment_link ? "stripe_payment_link" : (isDemoPayment ? "demo" : "other"),
      payment_status: paymentDetails.payment_link || isDemoPayment ? "payment_requested" : "not_requested",
    ...
    if (payment.payment_link && payment.email) {
      try {
...
function handlePortalAction_(spreadsheet, payload) {
  ...
  switch (action) {
    ...
    case "portal_complete_demo_payment":
  ...
  try {
    ...
    if (existing) {
      ...
      return {
        ...
        payment_url: buildStripePaymentUrl_(existing.data.payment_link, existing.data.payment_id),
    ...
    const paymentLinkSheet = getOrCreateSheet_(spreadsheet, CRM_PAYMENT_LINK_SHEET_NAME);
    setupPaymentLinksSheet_(paymentLinkSheet);
    const paymentLink = getSheetRecordsFromSheet_(paymentLinkSheet, PAYMENT_LINK_COLUMNS)
      .find((record) => normalizePaymentLinkOfferCode_(record.data.offer) === stage.offer &&
        normalizeValue_(record.data.status) === "active" &&
        Number(record.data.amount_cad) === Number(stage.amount_cad) &&
        normalizeValue_(record.data.stripe_payment_link));
    if (!paymentLink) {
      return { ok: false, code: "PAYMENT_LINK_NOT_CONFIGURED" };
    }
    ...
    const payment = {
      ...
      payment_link: paymentLink.data.stripe_payment_link,
      invoice_id: "",
      due_date: "",
      paid_at: "",
      tutor_payout_cad: "0",
    ...
    return {
      ...
      payment_url: buildStripePaymentUrl_(payment.payment_link, payment.payment_id),
...
function bookPortalSession_(spreadsheet, payload) {
  ...
  const paymentDetails = resolveSessionPaymentDetails_(spreadsheet, { session_type: sessionType });
  const paymentMode = planBinding.requires_credit ? "plan_credit" : (paymentDetails.payment_link ? "stripe" : "demo");
  const amountCad = paymentDetails.amount_cad || defaultSessionAmountCad_(sessionType);
  ...
  return {
    ...
    payment_link: paymentMode === "stripe" ? buildStripePaymentUrl_(record.payment_link, payment ? payment.payment_id : "") : "",
    amount_cad: record.amount_cad,
  };
}

function completePortalDemoPayment_(spreadsheet, payload) {
  const portalSession = verifyPortalSession_(spreadsheet, payload.token, "parent");
  if (!portalSession.ok) {
    return portalSession;
  }
  ...
  if (!sessionRecord || normalizeValue_(sessionRecord.data.payment_link) ||
      normalizeValue_(sessionRecord.data.credit_reservation_id)) {
    return { ok: false, code: "DEMO_PAYMENT_NOT_AVAILABLE" };
  }

  if (["paid", "demo_paid"].includes(normalizeValue_(sessionRecord.data.payment_status))) {
    return { ok: true, session_id: sessionId, already_paid: true };
  }

  const now = new Date().toISOString();
  const amountCad = normalizeValue_(sessionRecord.data.amount_cad) || defaultSessionAmountCad_(sessionRecord.data.session_type);
  const paymentSheet = getOrCreateSheet_(spreadsheet, CRM_PAYMENT_SHEET_NAME);
  setupPaymentSheet_(paymentSheet);
  const paymentRecord = getSheetRecordsFromSheet_(paymentSheet, PAYMENT_COLUMNS)
    .find((record) => normalizeValue_(record.data.session_id) === sessionId);
  const payment = {
    payment_id: paymentRecord ? paymentRecord.data.payment_id : createRecordId_("PAY"),
    session_id: sessionId,
    lead_id: sessionRecord.data.lead_id,
    parent_name: sessionRecord.data.parent_name,
    email: normalizeEmail_(portalSession.access.email),
    offer: sessionRecord.data.session_type,
    amount_cad: amountCad,
    payment_method: "demo",
    payment_status: "demo_paid",
    payment_link: "",
    invoice_id: "demo-payment",
    paid_at: now,
    payout_status: "not_due",
    notes: [normalizeValue_(paymentRecord?.data.notes), "Payment simulation only. No card was charged."].filter(Boolean).join(" | "),
    created_at: paymentRecord ? paymentRecord.data.created_at : now,
    updated_at: now,
  };
  writeRecord_(paymentSheet, PAYMENT_COLUMNS, paymentRecord ? paymentRecord.rowNumber : null, payment);
  ...
  return { ok: true, session_id: sessionId, payment_id: payment.payment_id, payment_status: "demo_paid", demo: true };
}
...
function sendPaymentRequestEmail_(payment) {
  const paymentUrl = buildStripePaymentUrl_(payment.payment_link, payment.payment_id);
...
function buildStripePaymentUrl_(paymentLink, paymentId) {
  const link = normalizeValue_(paymentLink);
  const reference = normalizeValue_(paymentId);
  if (!link || !reference) {
    return link;
  }

  return `${link}${link.includes("?") ? "&" : "?"}client_reference_id=${encodeURIComponent(reference)}`;
}
...
function sanitizePaymentForParent_(record) {
  ...
  return {
    ...
    amount_cad: record.amount_cad,
    payment_status: record.payment_status,
    payment_url: buildStripePaymentUrl_(record.payment_link, record.payment_id),
