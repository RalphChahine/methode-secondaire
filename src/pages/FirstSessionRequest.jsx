...
export default function FirstSessionRequest() {
  ...
  return <div className="reference-page relative overflow-hidden notebook-paper">
    ...
    <div className="relative z-10 mx-auto w-full max-w-6xl px-5 pb-20 pt-8 sm:px-6 lg:px-8 lg:pb-28 lg:pt-14"><div className="grid gap-8 lg:grid-cols-[0.78fr,1.1fr] lg:items-start">
      <section className="pt-2 lg:sticky lg:top-28">
        <div className="inline-flex items-center gap-2 rounded-full border border-ink/15 bg-white/60 px-4 py-1.5 text-sm text-ink/80"><BadgeCheck className="h-4 w-4 text-cobalt" />{requestCopy.badge}</div>
        <h1 className="balanced-copy mt-6 font-display text-4xl font-semibold leading-[0.98] text-ink sm:text-5xl">{requestCopy.title}</h1><p className="mt-5 max-w-xl text-base leading-8 text-ink/72 sm:text-lg">{requestCopy.description}</p>
        <div className="mt-6 inline-flex items-center gap-2 rounded-full border border-cobalt/25 bg-cloud px-4 py-2 text-sm font-semibold text-ink"><CalendarCheck className="h-4 w-4 text-cobalt" />{requestCopy.price}</div>
        <p className="mt-6 text-sm leading-6 text-ink/64">{requestCopy.callPrefix}</p><Button asChild variant="outline" className="notebook-button-secondary mt-3"><a href={`tel:${siteConfig.phone}`}><Phone className="h-4 w-4" />{requestCopy.call}</a></Button>
      </section>
      <MotionCard className="notebook-ink rounded-[32px] p-5 text-white sm:p-7"><div className="flex items-center gap-2 text-sm font-semibold text-sun"><ArrowRight className="h-4 w-4" />{requestCopy.formEyebrow}</div><h2 className="balanced-copy mt-3 font-display text-3xl font-semibold leading-tight">{requestCopy.formTitle}</h2><p className="mt-3 text-sm leading-7 text-white/72">{requestCopy.formDescription}</p><FirstSessionRequestForm className="mt-6" locale={locale} pageName={locale === "en" ? "first-session-request-en" : "first-session-request"} offer={requestedOffer} initialSubject={initialSubject} onSuccess={() => navigate(getLocalizedPath("thankYou", locale))} /><details className="mt-6 border-t border-white/10 pt-4"><summary className="cursor-pointer text-sm font-semibold text-white">{requestCopy.stepsTitle}</summary><ol className="mt-4 space-y-3">{requestCopy.steps.map((step, index) => <li key={step} className="flex items-start gap-3 text-sm leading-6 text-white/76"><span className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-sun text-xs font-bold text-ink">{index + 1}</span>{step}</li>)}</ol></details></MotionCard>
    </div></div>
  </div>
}
