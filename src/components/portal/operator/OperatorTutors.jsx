...
export default function OperatorTutors({ copy, tutors = [], selectedTutor, onSelect, createPanel, detail }) {
  const [query, setQuery] = useState("")
  const [status, setStatus] = useState("all")
  const [isCreating, setIsCreating] = useState(false)
  const filtered = useMemo(() => searchOperatorTutors(tutors, query).filter((tutor) => status === "all" || tutor.status === status), [query, status, tutors])
