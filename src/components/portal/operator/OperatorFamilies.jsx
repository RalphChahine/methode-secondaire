...
export default function OperatorFamilies({ copy, families = [], selectedFamily, onSelect, createPanel }) {
  const [query, setQuery] = useState("")
  const [isCreating, setIsCreating] = useState(false)
  const filtered = useMemo(() => searchOperatorFamilies(families, query), [families, query])
  ...
  return <PortalDetailPanel title={copy.childrenTitle || copy.parentName} description={copy.parentDashboard}>
    ...
    <div className="space-y-2">{filtered.length ? filtered.map((family) => <button key={family.lead_id || family.parent_id || family.email} type="button" onClick={() => onSelect?.(family)} className="flex min-h-16 w-full items-center justify-between gap-3 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-left hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#f5c977]"><span className="flex min-w-0 items-center gap-3"><UsersRound className="h-5 w-5 shrink-0 text-[#f5c977]" /><span className="min-w-0"><span className="block truncate text-sm font-semibold">{family.parent_name || family.email || copy.empty}</span><span className="mt-1 block truncate text-xs text-white/55">{family.student_name || family.student_level_subject || family.phone || ""}</span></span></span><span className="shrink-0 text-xs text-white/50">{family.lead_status || family.access_status || ""}</span></button>) : <p className="text-sm leading-6 text-white/60">{copy.empty}</p>}</div>
  </PortalDetailPanel>
}
